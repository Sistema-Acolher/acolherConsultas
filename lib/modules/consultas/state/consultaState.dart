...
class ConsultaState extends ChangeNotifier {
  ...
  // Notifiers
  ValueNotifier<bool> notifierRefeicoesComTecnologia = ValueNotifier<bool>(false);
  ...
  ValueNotifier<bool> notifierConsumiuVerdurasLegumes = ValueNotifier<bool>(false);
  ValueNotifier<bool> notifierConsumiuEmbutidos = ValueNotifier<bool>(false);
  ValueNotifier<bool> notifierConsumiuBebidasAdocicadas = ValueNotifier<bool>(false);
  ValueNotifier<bool> notifierConsumiuMacarraoInstantaneoSalgado = ValueNotifier<bool>(false);
  ValueNotifier<bool> notifierConsumiuBiscoitoRecheado = ValueNotifier<bool>(false);
  ...
  ValueNotifier<bool> notifierReflexoPreensaoPalmar = ValueNotifier<bool>(false);
  ValueNotifier<bool> notifierReflexoPreensaoPlantar = ValueNotifier<bool>(false);
  ValueNotifier<bool> notifierReflexoBabinski = ValueNotifier<bool>(false);
  ValueNotifier<bool> notifierReflexoTonicoCervical = ValueNotifier<bool>(false);
  ...
  ValueNotifier<bool> notifierMovimentosSimetricosFaciais = ValueNotifier<bool>(false);
  ValueNotifier<bool> notifierReflexoPiscarAcustico = ValueNotifier<bool>(false);
  ...
  ValueNotifier<bool> notifierRegularidadeMenstrual = ValueNotifier<bool>(false);
  ...
  Consulta cadastro() {
    Consulta consulta = Consulta(
      ...
      observacoesPaciente: observacoesPaciente.text.trim().isEmpty ? " " : observacoesPaciente.text.trim(),
      ...
      dataUltimaMenstruacao: dataUltimaMenstruacao.text.trim().isEmpty ? DateTime(1900, 1, 1) : DateFormat('dd/MM/yyyy').parse(dataUltimaMenstruacao.text),
      ...
      observacoesSaudeSexualEReprodutivaMeninas: observacoesSaudeSexualEReprodutivaMeninas.text.isEmpty ? " " : observacoesSaudeSexualEReprodutivaMeninas.text,
      ...
      quandoFezPreventivo: quandoFezPreventivo.text.trim().isEmpty ? DateTime(1900, 1, 1) : DateFormat('dd/MM/yyyy').parse(quandoFezPreventivo.text),
      ...
      quandoSemenarca: quandoSemenarca.text.trim().isEmpty ? DateTime(1900, 1, 1) : DateFormat('dd/MM/yyyy').parse(quandoSemenarca.text),
  ...
  cuidadorPrincipal.value = TextEditingValue(text: consulta.cuidadorPrincipal);
  ...
  observacoesPaciente.value = TextEditingValue(text: consulta.observacoesPaciente ?? '');
  refeicoesComTecnologia.value = TextEditingValue(text: consulta.refeicoesComTecnologia);
  refeicoesDuranteODia.value = TextEditingValue(text: consulta.refeicoesDuranteODia);
  ...
  consumiuVerdurasLegumes.value = TextEditingValue(text: consulta.consumiuVerdurasLegumes);
  consumiuEmbutidos.value = TextEditingValue(text: consulta.consumiuEmbutidos);
  consumiuBebidasAdocicadas.value = TextEditingValue(text: consulta.consumiuBebidasAdocicadas);
  consumiuMacarraoInstantaneoSalgado.value = TextEditingValue(text: consulta.consumiuMacarraoInstantaneoSalgado);
  consumiuBiscoitoRecheado.value = TextEditingValue(text: consulta.consumiuBiscoitoRecheado);
  ...
  comprimentoPorIdade.value = TextEditingValue(text: consulta.comprimentoPorIdade);
  imcPorIdade.value = TextEditingValue(text: consulta.imcPorIdade);
  frequenciaCardiaca.value = TextEditingValue(text: consulta.frequenciaCardiaca);
  ...
  avaliacaoMuscoesqueletica.value = TextEditingValue(text: consulta.avaliacaoMuscoesqueletica);
  ...
  observacoesDesenvolvimento.value = TextEditingValue(text: consulta.observacoesDesenvolvimento ?? '');
  comoSeSenteHoje.value = TextEditingValue(text: consulta.comoSeSenteHoje);
  observacoesPsicoemocionais.value = TextEditingValue(text: consulta.observacoesPsicoemocionais ?? '');
  ...
  frequenciaRespiratoria.value = TextEditingValue(text: consulta.frequenciaRespiratoria ?? '');
  bregmatica.value = TextEditingValue(text: consulta.bregmatica?.toString() ?? '');
  bregmaticaCalcificada.value = consulta.bregmaticaCalcificada ?? false;
  lambdoide.value = TextEditingValue(text: consulta.lambdoide?.toString() ?? '');
  lambdoideCalcificada.value = consulta.lambdoideCalcificada ?? false;
  avaliacaoLinguagem.value = TextEditingValue(text: consulta.avaliacaoLinguagem ?? '');
  ...
  reflexoPreensaoPalmar.value = TextEditingValue(text: consulta.reflexoPreensaoPalmar ?? '');
  reflexoPreensaoPlantar.value = TextEditingValue(text: consulta.reflexoPreensaoPlantar ?? '');
  reflexoBabinski.value = TextEditingValue(text: consulta.reflexoBabinski ?? '');
  reflexoTonicoCervical.value = TextEditingValue(text: consulta.reflexoTonicoCervical ?? '');
  ...
  reflexoPiscarOptico.value = TextEditingValue(text: consulta.reflexoPiscarOptico ?? '');
  reflexoBuscaESuccao.value = TextEditingValue(text: consulta.reflexoBuscaESuccao ?? '');
  movimentosSimetricosFaciais.value = TextEditingValue(text: consulta.movimentosSimetricosFaciais ?? '');
  reflexoPiscarAcustico.value = TextEditingValue(text: consulta.reflexoPiscarAcustico ?? '');
  ...
  // Campos somente adolescentes
  atividadesLazer.value = TextEditingValue(text: consulta.atividadesLazer ?? '');
  identidadeGenero.value = TextEditingValue(text: consulta.identidadeGenero ?? '');
  sexualidade.value = TextEditingValue(text: consulta.sexualidade ?? '');
  estagioTurnerMeninasMamas.value = TextEditingValue(text: consulta.estagioTurnerMeninasMamas ?? '');
  estagioTurnerMeninasPelosPubianos.value = TextEditingValue(text: consulta.estagioTurnerMeninasPelosPubianos ?? '');
  dataUltimaMenstruacao.value = TextEditingValue(text: consulta.dataUltimaMenstruacao != null ? DateFormat('dd/MM/yyyy').format(consulta.dataUltimaMenstruacao!) : '');
  fluxoMenstrual.value = TextEditingValue(text: consulta.fluxoMenstrual ?? '');
  regularidadeMenstruacao.value = TextEditingValue(text: consulta.regularidadeMenstruacao ?? '');
  ...
  usaMedicamento.value = TextEditingValue(text: consulta.usaMedicamento ?? '');
  qualMedicamento.value = TextEditingValue(text: consulta.qualMedicamento ?? '');
  observacoesSaudeSexualEReprodutivaMeninas.value = TextEditingValue(text: consulta.observacoesSaudeSexualEReprodutivaMeninas ?? '');
  vidaSexualAtiva.value = TextEditingValue(text: consulta.vidaSexualAtiva ?? '');
  usaMetodoContraceptivo.value = TextEditingValue(text: consulta.usaMetodoContraceptivo ?? '');
  qualMetodoContraceptivo.value = TextEditingValue(text: consulta.qualMetodoContraceptivo ?? '');
  jaFezPreventivo.value = TextEditingValue(text: consulta.jaFezPreventivo ?? '');
  quandoFezPreventivo.value = TextEditingValue(text: consulta.quandoFezPreventivo != null ? DateFormat('dd/MM/yyyy').format(consulta.quandoFezPreventivo!) : '');
  seMasturba.value = TextEditingValue(text: consulta.seMasturba ?? '');
  frequenciaMasturbacao.value = TextEditingValue(text: consulta.frequenciaMasturbacao ?? '');
  estagioTurnerMeninosGenitalia.value = TextEditingValue(text: consulta.estagioTurnerMeninosGenitalia ?? '');
  estagioTurnerMeninosPelosPubianos.value = TextEditingValue(text: consulta.estagioTurnerMeninosPelosPubianos ?? '');
  semenarca.value = TextEditingValue(text: consulta.semenarca ?? '');
  quandoSemenarca.value = TextEditingValue(text: consulta.quandoSemenarca != null ? DateFormat('dd/MM/yyyy').format(consulta.quandoSemenarca!) : '');
  analiseGeralCasa.value = TextEditingValue(text: consulta.analiseGeralCasa ?? '');
  exameFisicoCasa.value = TextEditingValue(text: consulta.exameFisicoCasa ?? '');
  avaliacoesCasa.value = TextEditingValue(text: consulta.avaliacoesCasa ?? '');
  oriParaCuidador.value = TextEditingValue(text: consulta.oriParaCuidador ?? '');
  oriParaPaciente.value = TextEditingValue(text: consulta.oriParaPaciente ?? '');
  oriParaCoordenacao.value = TextEditingValue(text: consulta.oriParaCoordenacao ?? '');
}
